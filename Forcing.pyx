...
import numpy as np
import cython
from libc.math cimport fabs, sin, cos, exp, fmin, fmax
...
cdef class Forcing:
    ...
    cpdef initialize(self, Grid.Grid Gr, ReferenceState.ReferenceState Ref, Th, NetCDFIO_Stats NS, ParallelMPI.ParallelMPI Pa):
        self.scheme.initialize(Gr, Ref, Th, NS, Pa)
        return
...
cdef class ForcingIsdac:
    ...
    cpdef initialize(self, Grid.Grid Gr, ReferenceState.ReferenceState Ref, Th, NetCDFIO_Stats NS, ParallelMPI.ParallelMPI Pa):
        ...
        for k in xrange(Gr.dims.nlg[2]):
            ...
            T, ql = sat_adjst(Ref.p0_half[k], thetal[k], self.initial_qt[k], Th)
            self.initial_entropy[k] = Th.entropy(Ref.p0_half[k], T, self.initial_qt[k], ql, 0.0)
...
cdef class ForcingIsdacCC:
    def __init__(self, namelist):
        ...
        try:
            self.dz_inv = namelist['initial']['dzi'] #inversion depth
        except:
            self.dz_inv = 30.0
    ...
    cpdef initialize(self, Grid.Grid Gr, ReferenceState.ReferenceState Ref, Th, NetCDFIO_Stats NS, ParallelMPI.ParallelMPI Pa):
        ...
        for k in xrange(Gr.dims.nlg[2]):
            ...
            #Now get entropy profile
            T, ql = sat_adjst(Ref.p0_half[k], Ref.ic_thetal[k], Ref.ic_qt[k], Th)
            self.initial_entropy[k] = Th.entropy(Ref.p0_half[k], T, Ref.ic_qt[k], ql, 0.0)
            self.initial_qt[k] = Ref.ic_qt[k]
...
cdef class ForcingMpace:
    ...
    cpdef initialize(self, Grid.Grid Gr, ReferenceState.ReferenceState Ref, Th, NetCDFIO_Stats NS, ParallelMPI.ParallelMPI Pa):
        ...
        cdef:
            Py_ssize_t k
            double omega_min = self.divergence*(Ref.Pg - 85000.0)
        ...
        with nogil:
            for k in range(Gr.dims.nlg[2]):
                self.dtdt[k] = fmin(-4.0, -15.0*(1.0 - (Ref.Pg - Ref.p0_half[k])/21818.0))/86400.0
                self.dqtdt[k] = fmin(-0.164, -3.0*(1.0 - (Ref.Pg - Ref.p0_half[k])/15171.0))/86400000.0
                self.subsidence[k] = -fmin(self.divergence*(Ref.Pg - Ref.p0_half[k]), omega_min) / Ref.rho0_half[k] / g
